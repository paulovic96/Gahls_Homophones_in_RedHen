...
CELEX_FILE = "Data/epw.cd"
BERNDT_CHARACTER_CODING_FILE = "Data/celex_phonetic_character_code_berndt1987.csv"
BERNDT_CONDITIONAL_PROB_TABLE_FILE = "Data/Conditional_Probabilities_for_Grapheme-to-Phoneme_Correspondences_Berndt1987.csv"
...
def get_celex_transcription(df, celex_phonology_dict):
    return df.merge(celex_phonology_dict[["word", "disc", "clx", "disc_no_bound", "clx_no_bound"]],
                    how = "left",
                    left_on=["word", "celexPhon"],
        ...
        if not p is np.nan:
            ...
            if len(p) > 1:
                for p_i in p:
                    ...
                    """
                    berndt_arpabbet_phon_dict[p_i] = berndt_character_code_df.keyboard_compatible_phonetic_symbol.iloc[i]
            else:
                """
                ...
                """
                berndt_arpabbet_phon_dict[p[0]] = berndt_character_code_df.keyboard_compatible_phonetic_symbol.iloc[i]
    return berndt_arpabbet_phon_dict
...
def get_grapheme_string_with_conditional_prob_for_keyboard_phonetics(word_phon_tuple, phonem_graphem_prob_dict):
    ...
    for i, word_pron in enumerate(word_phon_tuple):
            ...
            if isinstance(p, list):
                for p_i in p:
                    if isinstance(p_i, list):
                        for p_ij in p_i:
                            for possible_grapheme in phonem_graphem_prob_dict[
                                p_ij]:  # all possible corresponding graphemes
                        ...
                        for possible_grapheme in phonem_graphem_prob_dict[
                            p_i]:  # all possible corresponding graphemes
                            grapheme = possible_grapheme[0].split("-")  # account for silent e encoded by e.g. A-E
                            prior = possible_grapheme[1]  # prior prob
                            cond = possible_grapheme[2]  # cond prop
                ...
                for possible_grapheme in phonem_graphem_prob_dict[
                    p]:  # all possible corresponding graphemes
                    grapheme = possible_grapheme[0].split("-")  # account for silent e encoded by e.g. A-E
                    prior = possible_grapheme[1]  # prior prob
                    cond = possible_grapheme[2]  # cond prop
                    for k, word_rest in enumerate(
